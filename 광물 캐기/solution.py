...
자원을 5개 단위로 나누어 그 묶음을 다이아 곡갱이로 캘 때, 철 곡갱이로 캘 때, 돌 곡갱이로 캘 때 피로도를 계산하여 리스트로 따로 저장해줍니다. [다이아 곡갱이로 5개를 다 캘 때 피로도, 철 곡갱이로 다 캘 때 피로도, 돌 곡갱이로 다 캘 때 피로도]를 저는 info라는 리스트에 넣어줬습니다. 이 것을 minerals의 끝까지 돌려줍니다. -> 첫 번째 예시의 경우 info = [[5, 17, 85], [3, 7, 31]] 이런 식으로 넣어집니다.

리스트 안의 값을 보았을 때 돌 곡갱이로 캘 때 피로도가 제일 큰 값인 경우 -> 다이아 곡갱이로 캐는 게 효율이 좋습니다.
위의 예시로 보면 info에 첫 번째 묶음은 [5,17,85]이고 두 번째 묶음은 [3,7,31]인데 첫 번째 묶음의 경우 돌 곡갱이로 캐는 경우 피로도가 85이므로 가지고 있는 가장 좋은 곡갱이로 캐는 게 효율이 좋습니다. 이를 한번의 알 기 위해 info 리스트를 돌 곡갱이 피로도, 철 곡갱이 피로도, 다이아 곡갱이 피로도 순으로 info를 정렬을 해줍니다.
